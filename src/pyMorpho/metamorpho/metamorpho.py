...
class MetaMorpho(ERC4626):
    def __init__(
        ...
        self.pending_cap: defaultdict[bytes, PendingUint192] = defaultdict(
            PendingUint192
        )
        ...
        Mixer.contracts_and_eoas[asset].force_approve(
            self.MORPHO, 2**256 - 1, self.metadata.address
        )
    ...
    def _only_allocator_role(self, sender):
        assert (
            self.is_allocator[sender] or sender == self.owner
        ), ErrorsLib.NotAllocatorRole

    def _only_guardian_role(self, sender):
        assert (
            sender == self.guardian or sender == self.owner
        ), ErrorsLib.NotGuardianRole

    def _only_curator_or_guardian_role(self, sender):
        assert (
            sender == self.curator or sender == self.guardian or sender == self.owner
    ...
    def submit_timelock(self, new_timelock: int, sender=Mixer.ZERO_ADDRESS):
        ...
        if new_timelock > self.timelock:
            ...
            assert not (
                new_timelock == self.pending_timelock.value
    ...
    def set_fee(self, new_fee: int, sender=Mixer.ZERO_ADDRESS):
        ...
        assert not (
            new_fee != 0 and self.fee_recipient == Mixer.ZERO_ADDRESS
    ...
    def set_fee_recipient(self, new_fee_recipient, sender=Mixer.ZERO_ADDRESS):
        ...
        assert not (
            new_fee_recipient == Mixer.ZERO_ADDRESS and self.fee != 0
        ), ErrorsLib.ZeroFeeRecipient
    ...
    def submit_guardian(self, new_guardian: Address, sender=Mixer.ZERO_ADDRESS):
        ...
        if self.guardian == Mixer.ZERO_ADDRESS:
            ...
            assert not (
                self.pending_guardian.valid_at != 0
                and new_guardian == self.pending_guardian.value
            ), ErrorsLib.AlreadyPending
    ...
    def submit_cap(
        ...
        assert not (
            Mixer.contracts_and_eoas[self.MORPHO].last_update(id, self.metadata.address)
            == 0
        ...
        if new_supply_cap < supply_cap:
            ...
            assert not (
                new_supply_cap == self.pending_cap[id].value
            ), ErrorsLib.AlreadyPending
    ...
    def submit_market_removal(self, id: bytes, sender=Mixer.ZERO_ADDRESS):
        ...
        self._set_cap(id, 0)
        self.config[id].removable_at = (
            Mixer.block_timestamp(self.metadata.chain) + self.timelock
        )
        # TODO: emit event ?
    ...
    def set_supply_queue(
        ...
        for i in range(length):
            assert not (
                self.config[new_supply_queue[i]].cap == 0
            ), ErrorsLib.MarketNotCreated
    ...
    def update_withdraw_queue(self, indexes: list[int], sender=Mixer.ZERO_ADDRESS):
        ...
        for i in range(curr_length):
            if not (seen[i]):
                id = self.withdraw_queue[i]
                assert not (
                    self.config[id].cap != 0
                ), ErrorsLib.InvalidMarketRemovalNonZeroCap(id)

                if (
                    Mixer.contracts_and_eoas[self.MORPHO].supply_shares(
    ...
    def skim(self, token: Address, sender=Mixer.ZERO_ADDRESS):
        ...
        Mixer.contracts_and_eoas[token].safe_transfer(
            self.skim_recipient, amount, self.metadata.address
        )
    ...
    def total_assets(self) -> int:
        ...
        for i in range(len(self.withdraw_queue)):
            assets += Mixer.contracts_and_eoas[self.MORPHO].expected_supply_assets(
                self._market_params(self.withdraw_queue[i]), self.metadata.address
            )
        return assets
    ...
    def _accrued_supply_balance(
        ...
        Mixer.contracts_and_eoas[self.MORPHO].accrue_interest(
            market_params, self.metadata.address
        )
        market = Mixer.contracts_and_eoas[self.MORPHO].market(id)
        shares = Mixer.contracts_and_eoas[self.MORPHO].supply_shares(
            id, self.metadata.address
        )
    ...
    def _set_cap(self, id: bytes, supply_cap: int):
        ...
        if supply_cap > 0:
            if not (market_config.enabled):
                ...
                assert not (
                    len(self.supply_queue) > ConstantsLib.MAX_QUEUE_LENGTH
                ), ErrorsLib.MaxQueueLengthExceeded
    ...
    def _simulate_withdraw_morpho(self, assets: int) -> int:
        for i in range(len(self.withdraw_queue)):
            id = self.withdraw_queue[i]
            market_params = self._market_params(id)
            supply_shares = Mixer.contracts_and_eoas[self.MORPHO].supply_shares(
                id, self.metadata.address
            )
            (
                total_supply_assets,
                ...
                total_borrow_assets,
                _,
            ) = Mixer.contracts[self.MORPHO].expected_market_balances(market_params)
    ...
    def _withdrawable(
        ...
        available_liquidity = UtilsLib.min(
            ...
            Mixer.contracts_and_eoas[market_params.loan_token].balance_of(self.MORPHO),
        )
    ...
    def _accrue_fee(self) -> int:
        ...
        fee_shares, new_total_assets = self._accrued_fee_shares()
        if fee_shares != 0:
            self._mint(self.fee_recipient, fee_shares)
        # TODO: emit event ?
    ...
    def _accrued_fee_shares(self) -> Tuple[int, int]:
        ...
        total_interest = UtilsLib.zero_floor_sub(
            new_total_assets, self.last_total_assets
        )
