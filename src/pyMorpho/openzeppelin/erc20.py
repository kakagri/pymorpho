...
class ERC20(ABC):
    def __init__(
        ...
        metadata: Metadata = Metadata(
            ChainID.ETH_MAINNET, Address.ZERO_ADDRESS, "ERC20", InstanceType.CONTRACT
        ),
    ...
    def transfer(
        self, to: Address, amount: int, sender: Address = Mixer.ZERO_ADDRESS
    ) -> bool:
    ...
    def approve(
        self, spender: Address, amount: int, sender: Address = Mixer.ZERO_ADDRESS
    ) -> bool:
    ...
    def increase_allowance(
        self, spender: Address, added_value: int, sender: Address = Mixer.ZERO_ADDRESS
    ) -> bool:
    ...
    def decrease_allowance(
        self,
        spender: Address,
        substracted_value: int,
        sender: Address = Mixer.ZERO_ADDRESS,
    ) -> bool:
        current_allowance = self.allowance(sender, spender)
        assert (
            current_allowance >= substracted_value
